...
/**
 * Confirms if WordPress is using pretty permalinks
 *
 * @since bbPress (r3471)
 *
 * @global WP_Rewrite $wp_rewrite The WordPress rewrite object
 * @return boolean True if using pretty permalinks, False if default
 */
function bbp_using_permalinks() {
	global $wp_rewrite;

	// Assume default permalinks
	$retval = false;

	// Set retval to true if using pretty sermalinks
...
/**
 * Get the permalink root if using pretty permalinks
 *
 * @since bbPress (r3471)
 *
 * @global WP_Rewrite $wp_rewrite
 * @return mixed String if using pretty permalinks, False if not
 */
function bbp_permalink_root() {
	global $wp_rewrite;

	// Use the root or an empty string
	$retval = bbp_using_permalinks() ? $wp_rewrite->root : false;

	return $retval;
}

/**
 * Get the permalink pagination base if using pretty permalinks
 *
 * @since bbPress (r3471)
 *
 * @global WP_Rewrite $wp_rewrite
 * @return mixed String if using pretty permalinks, False if not
 */
function bbp_permalink_pagination_base() {
	global $wp_rewrite;

	// Use the root or an empty string
	$retval = bbp_using_permalinks() ? $wp_rewrite->pagination_base : false;
