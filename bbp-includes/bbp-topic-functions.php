 ...
 * Insert a topic.
 *
 * If $topicarr has 'ID' set to a value, then topic will be updated.
 *
 * You can set the topic date manually, by setting the values for 'post_date'
 * and 'post_date_gmt' keys. You can open or close replies by setting the value
 * for 'post_status' key.
 *
 ...
 */
function bbp_insert_topic( $post_args = '', $meta_args = '' ) {
	global $bbp, $wpdb, $wp_rewrite;

	// Default post fields and post meta
	$post_defaults = array(

		// Pass a topic_id if you want to update rather than insert
		'topic_id'              => 0,
		...
		// Ping fields - (Not yet used by bbPress core)
	...
	// Parse
	$p = wp_parse_args( $post_args, $post_defaults );
	extract( $p, EXTR_SKIP );

	// Additional parameters for bbPress topics
	$meta_defaults = array(

		// 'forum_id' typically be synced with post_parent above
		'forum_id'              => 0,
		...
		// Reply and voice counts
		'anonymous_reply_count' => 0,
		'reply_count'           => 0,
	...
	// Parse
	$m = wp_parse_args( $meta_args, $meta_defaults );
	extract( $m, EXTR_SKIP );

	// Handle the post insertion
	$topic = wp_insert_post( $r );

	//
	foreach( $m as $meta_key => $meta_value ) {
		update_post_meta( $topic->ID, '_bbp_' . $meta_key, $meta_value );
	}
	bbp_update_topic_last_active_id( $topic->ID, $last_active_id );

	$post = wp_insert_post( $r );
